...
def loadfoldlist(obj, fold, quat=''):
    if 'Pheme' in obj or 'twitter' in obj:
        if quat != '':
            ...
            testLabel = os.path.join(cwd,'dataset/{}/split_{}/test.label.txt'.format(obj, fold))
        else:
            trainLabel = os.path.join(cwd,'dataset/{}/split_{}/train.label.txt'.format(obj, fold))
            testLabel = os.path.join(cwd,'dataset/{}/split_{}/test.label.txt'.format(obj, fold))
        ...
        print('loading tree label from split', fold)

        trainlist, testlist = [], []
        NR, R = 0, 0
        TR, FR, UR = 0, 0, 0
        for line in open(trainLabel):
            label, eid = line.rstrip().split('\t')[0], line.rstrip().split('\t')[-1]
        ...
        logger.info('TrainData | nonR: {}, R: {} (TR: {}, FR: {}, UR {})'.format(NR, R, TR, FR, UR))
        ...
        logger.info('TestData | nonR: {}, R: {} (TR: {}, FR: {}, UR {})'.format(NR, R, TR, FR, UR))
...
## For random 5 fold ##
#######################
def load5foldData(obj, random_seed):
    if 'Twitter' in obj:
        labelPath = os.path.join(cwd,"dataset/" +obj+"/"+ obj + "_label_All.txt")
        print(cwd)
        ...
        for line in open(labelPath):
            ...
            if labelDic[eid] in labelset_u:
                ...
                l4 += 1
        print(len(labelDic))
        print('Non-Rumor:{}, False:{}, True:{}, Unverified:{}'.format(l1,l2,l3,l4))
